...
def enqueue_embedding_job(job_id: str):
    """Enqueue durable job_id for an RQ worker.

    TransientJobError raised from process_job triggers Retry.
    Permanent failures mark Postgres failed and return without raising.
    """
    queue = get_queue()
