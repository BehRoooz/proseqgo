#!/usr/bin/env python3
"""RQ worker entrypoint for training jobs.

Starts Prometheus metrics, requeues orphaned started jobs, then runs RQ worker.
"""
...
from queueing import enqueue_training_job
from worker import sync_queue_gauges

logging.basicConfig(level=logging.INFO, format="%(asctime)s %(levelname)s %(message)s")
logger = logging.getLogger("training-rq-worker")
...
def requeue_orphaned_started_jobs(queue: Queue) -> int:
    ...
    for rq_job_id in list(registry.get_job_ids()):
        ...
        enqueue_training_job(durable_id)
        requeued += 1
...
def main() -> int:
    ...
    worker = Worker([queue], connection=redis, name="training-worker")
